...
def embedding(inputs,vocab_size,embed_dim,use_pretrain=False,scale=False,scope="embedding",reuse=None):
    """
    embedding look up to get embedding for each batch!
    :param inputs: batch data int.
    :param vocab_size: to init embedding if we don't use pre-trained embedding
    :param embed_dim: dim of embedding
    :param scale: true or false
    :param scope: scope name
    :param reuse: None means use their father's scope
    :return: outputs
    """
    with tf.variable_scope(scope,reuse=reuse):
        if use_pretrain:
            lookup_table = np.load(config.trimed_path)
            lookup_table = tf.get_variable('lookup_table',dtype=tf.float32,
                                           initializer=lookup_table)
        else:
            lookup_table = tf.get_variable('lookup_table',dtype=tf.float32,
                                       shape=[vocab_size,embed_dim],
                                       initializer=initializers.xavier_initializer())
        lookup_table = tf.concat((tf.zeros(shape=[1,embed_dim]),lookup_table[1:,:]),axis=0)
